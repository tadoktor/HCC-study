...
##Selecting only the HCC chemicals
HCC_chem_data2 <- read.table(paste(Root_location,"/InputFiles/Non_genotoxic_carcinogens_Advance.txt",sep=''),sep='\t',header=T,comment.char='',strip.white=T)
HCC_chem_list2 <- na.omit(HCC_chem_data2[,3])
...
Gene_data_Toxcast_human_hcc <- which(Gene_data_Toxcast_human[,5] %in% c(HCC_chem_list2))
all_ids <- Gene_data_Toxcast_human[,c(1:5)]
Gene_data_Toxcast_humanActive <-  Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_hcc),]
Gene_data_Toxcast_human_unique <- unique(Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_hcc),c(-1,-2,-3,-4)])
Gene_data_Toxcast_human <- Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_hcc),c(-1,-3,-4,-5)]
...
dataset <- "/OutputFiles/Toxcast_Active_dataset_HCC_chem.txt"
...
####selecting for hcc chemicals
Gene_data_CTD_TC_human <- unique(merge(Idcols, Gene_data_CTD_TC_human, by.x = 'cid', by.y = 'cid'))
Gene_data_CTD_TC_human_hcc <- which(Gene_data_CTD_TC_human[,5] %in% c(HCC_chem_list2))
Gene_data_CTD_TC_human <- Gene_data_CTD_TC_human[c(Gene_data_CTD_TC_human_hcc),]#c(-1,-2,-3,-4,-5,-6,-7)]
...
dataset <- "/OutputFiles/CTDTC_Active_dataset_HCC_chem.txt"
...
Gene_data_CTDALL_hcc <- which(Gene_data_CTDALL[,5] %in% c(HCC_chem_list2))
CTD_all_hcc <- Gene_data_CTDALL[c(Gene_data_CTDALL_hcc),]#c(-1,-2,-3,-4,-5,-6,-7)]
Gene_data_CTDALL_humanActive <- CTD_all_hcc
CTD_all_hcc_unique <- unique(CTD_all_hcc[,c(-2,-3,-4,-5)])
CTD_all_hcc <- unique(CTD_all_hcc[,c(-2,-3,-4,-5)])
cid <- CTD_all_hcc[,1]
CTD_all_hcc <- CTD_all_hcc[,c(-1,-2,-3)]
##making sure all values are discretized either 1 or 0
CTD_all_hcc[CTD_all_hcc>=1] <- 1
CTD_all_hcc[CTD_all_hcc<1] <- 0
CTD_all_hcc <- cbind(cid, CTD_all_hcc)
...
dataset <- "/OutputFiles/CTDALL_Active_dataset_HCC_chem.txt"
...
Disease_data_CTD_hcc <- which(Disease_data_CTD[,5] %in% c(HCC_chem_list2))
Disease_data_CTD_Unique <- unique(Disease_data_CTD[c(Disease_data_CTD_hcc),c(-2,-3,-4,-5,-7)])
Disease_data_CTD <- Disease_data_CTD[c(Disease_data_CTD_hcc),c(-2,-3,-4,-5,-6,-7)]
...
logfile1<-paste(Root_location,"/OutputFiles/TCgenes_in_CTD_HCCchem_logfile",today,".txt",sep="")
logfile2<-paste(Root_location,"/OutputFiles/allCTD_HCCchem_logfile",today,".txt",sep="")
logfile3<-paste(Root_location,"/OutputFiles/Toxcast_HCCchem_logfile",today,".txt",sep="")
...
rules1<-arules_function(Gene_data_CTD_TC_human,Disease_data_CTD,10,1,logfile1)#,"CTDTC_merge_HCC.txt")
rules2<-arules_function(CTD_all_hcc,Disease_data_CTD,10,1,logfile2)#, "CTDALL_merge_HCC.txt")
rules3<-arules_function(Gene_data_Toxcast_human,Disease_data_CTD,10,1,logfile3) #,"Toxcast_merge_HCC.txt")

write.table(rules1,paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_HCCchem_assoc_rules",today,".txt",sep=''),sep="\t",row.names=F)
write.table(rules2,paste(Root_location,"/OutputFiles/workflow4_AllCTD_HCCchem_assoc_rules",today,".txt",sep=''),sep="\t",row.names=F)
write.table(rules3,paste(Root_location,"/OutputFiles/workflow4_Toxcast_HCCchem_assoc_rules",today,".txt",sep=''),sep="\t",row.names=F)
...
write.table(Toxcast_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_Toxcast_HCCchem_assoc_rules_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
CTD_TC_Results_wtDisease_map <- unique(merge(rules1, Disease_map, by.x='rhs', by.y='DiseaseID'))
write.table(CTD_TC_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_HCCchem_assoc_rules_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
CTD_all_Results_wtDisease_map <- unique(merge(rules2, Disease_map, by.x='rhs', by.y='DiseaseID'))
write.table(CTD_all_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_allCTD_HCCchem_assoc_rules_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
