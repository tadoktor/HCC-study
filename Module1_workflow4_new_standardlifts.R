...
##Selecting only the steatosis chemicals
#Steatosis_chem_data <- read_excel(paste(Root_location,"/InputFiles/Non_genotoxic_carcinogens_Advance.xlsx",sep=''),sheet=1)
#Steatosis_chem_list <- na.omit(Steatosis_chem_data[,3])
Steatosis_chem_data2 <- read.table(paste(Root_location,"/InputFiles/Non_genotoxic_carcinogens_Advance.txt",sep=''),sep='\t',header=T,comment.char='',strip.white=T)
Steatosis_chem_list2 <- na.omit(Steatosis_chem_data2[,3])


Gene_data_Toxcast_human <- read.csv(paste(Root_location,"/IntermediateData/ToxCast_chem_x_gene_Hitcalls_wt_all_IDs_human.csv", sep=''),sep=',', header=TRUE, comment.char='', strip.white=TRUE)
Idcols <- Gene_data_Toxcast_human[,c(1:5)]

Gene_data_Toxcast_human_steatosis <- which(Gene_data_Toxcast_human[,5] %in% c(Steatosis_chem_list2))
all_ids <- Gene_data_Toxcast_human[,c(1:5)]
Gene_data_Toxcast_humanActive <-  Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_steatosis),]
Gene_data_Toxcast_human_unique <- unique(Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_steatosis),c(-1,-2,-3,-4)])
Gene_data_Toxcast_human <- Gene_data_Toxcast_human[c(Gene_data_Toxcast_human_steatosis),c(-1,-3,-4,-5)]
...
#Gene_data_Toxcast_humanActive2 <- Gene_data_Toxcast_humanActive[,c(-1)]
#Gene_data_Toxcast_humanActive2 <- cbind(ids2, Gene_data_Toxcast_humanActive2)
...
dataset <- "/OutputFiles/Toxcast_Active_dataset.txt"
...
####selecting for steatosis chemicals
Gene_data_CTD_TC_human <- unique(merge(Idcols, Gene_data_CTD_TC_human, by.x = 'cid', by.y = 'cid'))
Gene_data_CTD_TC_human_steatosis <- which(Gene_data_CTD_TC_human[,5] %in% c(Steatosis_chem_list2))
#Gene_data_CTD_TC_human <- which(Gene_data_CTD_TC_human[,1] %in% c(Gene_data_Toxcast_human[,1]))
#cid <- Gene_data_CTD_TC_human[,1]
Gene_data_CTD_TC_human <- Gene_data_CTD_TC_human[c(Gene_data_CTD_TC_human_steatosis),]#c(-1,-2,-3,-4,-5,-6,-7)]
Gene_data_CTD_TC_humanActive <- Gene_data_CTD_TC_human#1[c(Gene_data_CTD_TC_human_steatosis),]
#cid <- Gene_data_CTD_TC_human[,1]
Gene_data_CTD_TC_human_unique <- unique(Gene_data_CTD_TC_human[,c(-2,-3,-4,-5)])
Gene_data_CTD_TC_human <- unique(Gene_data_CTD_TC_human[,c(-2,-3,-4,-5)])
cid <- Gene_data_CTD_TC_human[,1]
...
ids3 <- Gene_data_CTD_TC_humanActive[,1:7]
Gene_data_CTD_TC_humanActive2 <- Gene_data_CTD_TC_humanActive[,c(-1,-2,-3,-4,-5,-6,-7)]
##making sure all values are discretized either 1 or 0
...
#Gene_data_Toxcast_humanActive2 <- Gene_data_Toxcast_humanActive[,c(-1)]
#Gene_data_Toxcast_humanActive2 <- cbind(ids2, Gene_data_Toxcast_humanActive2)
uniqueIDs <- as.data.frame(matrix(c(1:58),nrow=nrow(ids3),ncol=1) )
...
colnames(uniqueIDs)<-c("UID","PubChem_CID", "PubChem_SID.TC","Tox21","Casrn_code","DSSTox_GSID","CTDID","PubChem_SID.CTD")
CTDTC_Active_dataset <- merge(uniqueIDs, CTDTC_Active_dataset, by.x='UID', by.y='uid')
dataset <- "/OutputFiles/CTDTC_Active_dataset.txt"
...
Gene_data_CTDALL <- unique(merge(Idcols, CTD_all, by.x = 'cid', by.y = 'cid'))
Gene_data_CTDALL_steatosis <- which(Gene_data_CTDALL[,5] %in% c(Steatosis_chem_list2))
#CTD_all_steatosis <- which(CTD_all[,3] %in% c(Steatosis_chem_list2))
CTD_all_steatosis <- Gene_data_CTDALL[c(Gene_data_CTDALL_steatosis),]#c(-1,-2,-3,-4,-5,-6,-7)]
Gene_data_CTDALL_humanActive <- CTD_all_steatosis
#CTD_all<-na.omit(CTD_all1)
CTD_all_steatosis_unique <- unique(CTD_all_steatosis[,c(-2,-3,-4,-5)])
CTD_all_steatosis <- unique(CTD_all_steatosis[,c(-2,-3,-4,-5)])
cid <- CTD_all_steatosis[,1]
CTD_all_steatosis <- CTD_all_steatosis[,c(-1,-2,-3)]
##making sure all values are discretized either 1 or 0
CTD_all_steatosis[CTD_all_steatosis>=1] <- 1
CTD_all_steatosis[CTD_all_steatosis<1] <- 0
CTD_all_steatosis <- cbind(cid, CTD_all_steatosis)
...
ids4 <- Gene_data_CTDALL_humanActive[,1:7]
Gene_data_CTDALL_humanActive2 <- Gene_data_CTDALL_humanActive[,c(-1,-2,-3,-4,-5,-6,-7)]
##making sure all values are discretized either 1 or 0
...
colnames(uniqueIDs2)<-c("UID","PubChem_CID", "PubChem_SID.TC","Tox21","Casrn_code","DSSTox_GSID","CTDID","PubChem_SID.CTD")
CTDALL_Active_dataset <- merge(uniqueIDs2,CTDALL_Active_dataset, by.x='UID', by.y='uid')
dataset <- "/OutputFiles/CTDALL_Active_dataset.txt"
...
Disease_data_CTD <- merge(Idcols, Disease_data_CTD, by.x = 'cid', by.y = 'cid')
Disease_data_CTD_steatosis <- which(Disease_data_CTD[,5] %in% c(Steatosis_chem_list2))
Disease_data_CTD_Unique <- unique(Disease_data_CTD[c(Disease_data_CTD_steatosis),c(-2,-3,-4,-5,-7)])
Disease_data_CTD <- Disease_data_CTD[c(Disease_data_CTD_steatosis),c(-2,-3,-4,-5,-6,-7)]
...
logfile1<-paste(Root_location,"/OutputFiles/TCgenes_in_CTD_new_standard_lifts",today,".txt",sep="")
logfile2<-paste(Root_location,"/OutputFiles/allCTD_new_standard_lifts",today,".txt",sep="")
logfile3<-paste(Root_location,"/OutputFiles/Toxcast_new_standard_lifts",today,".txt",sep="")
#logfile4<-paste(Root_location,"/OutputFiles/allCTD_new_standard_lifts_newfilter5",today,".txt",sep="")
logfile5<-paste(Root_location,"/OutputFiles/allCTD_new_standard_lifts_newfilter7",today,".txt",sep="")

rules1<-arules_function(Gene_data_CTD_TC_human,Disease_data_CTD,10,1,logfile1,"CTDTC_merge.txt")
rules2<-arules_function(CTD_all_steatosis,Disease_data_CTD,10,1,logfile2, "CTDALL_merge.txt")
rules3<-arules_function(Gene_data_Toxcast_human,Disease_data_CTD,10,1,logfile3,"Toxcast_merge.txt")
#rules4<-arules_function(CTD_all_steatosis,Disease_data_CTD,5,2,logfile4)
rules5<-arules_function(CTD_all_steatosis,Disease_data_CTD,7,2,logfile5, "CTDALL_merge_newfilter7.txt")

write.table(rules1,paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_new_stdlifts",today,".txt",sep=''),sep="\t",row.names=F)
write.table(rules2,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts",today,".txt",sep=''),sep="\t",row.names=F)
write.table(rules3,paste(Root_location,"/OutputFiles/workflow4_Toxcast_new_stdlifts",today,".txt",sep=''),sep="\t",row.names=F)
#write.table(rules4,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_newfilter5",today,".txt",sep=''),sep="\t",row.names=F)
write.table(rules5,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_newfilter7",today,".txt",sep=''),sep="\t",row.names=F)

#header <- as.data.frame((rules5))
#header$source <- sapply(strsplit(as.character(header[,8]),'[.]'), "[", 1)
#header$rhs <- sapply(strsplit(as.character(header[,8]),'[.]'), "[", 2)
#genesAt2$assay_component_endpoint_name <- sapply(strsplit(as.character(genesAt2[,1]),'[.]'), "[", 4)


######Merging disease names with modified result file
Toxcast_Results_file <- read.table(paste(Root_location,"/OutputFiles/workflow4_Toxcast_new_stdlifts_February_02_2018_modified.txt", sep=''),sep='\t', header=TRUE, comment.char='', strip.white=TRUE)
CTD_TC_Results_file <- read.table(paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_new_stdlifts_February_02_2018_modified.txt", sep=''),sep='\t', header=TRUE, comment.char='', strip.white=TRUE)
CTD_all_Results_file <- read.table(paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_February_02_2018_modified.txt", sep=''),sep='\t', header=TRUE, comment.char='', strip.white=TRUE)
...
write.table(Toxcast_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_Toxcast_new_stdlifts_wtDiseaseMap",today,".txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
CTD_TC_Results_wtDisease_map <- unique(merge(rules1, Disease_map, by.x='rhs', by.y='DiseaseID'))
write.table(CTD_TC_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_new_stdlifts_wtDiseaseMap",today,".txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
CTD_all_Results_wtDisease_map <- unique(merge(rules2, Disease_map, by.x='rhs', by.y='DiseaseID'))
write.table(CTD_all_Results_wtDisease_map_newfilter7,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_wtDiseaseMap",today,".txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
CTD_all_Results_wtDisease_map_newfilter7 <- unique(merge(rules5, Disease_map, by.x='rhs', by.y='DiseaseID'))
write.table(CTD_all_Results_wtDisease_map_newfilter7,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_newfilter7_wtDiseaseMap",today,".txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)


#Toxcast_Results_wtDisease_map <- unique(merge(Toxcast_Results_file, Disease_map, by.x='rhs', by.y='DiseaseID'))
#write.table(Toxcast_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_Toxcast_new_stdlifts_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
#CTD_TC_Results_wtDisease_map <- unique(merge(CTD_TC_Results_file, Disease_map, by.x='rhs', by.y='DiseaseID'))
#write.table(CTD_TC_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_new_stdlifts_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
#CTD_all_Results_wtDisease_map <- unique(merge(CTD_all_Results_file, Disease_map, by.x='rhs', by.y='DiseaseID'))
#write.table(CTD_all_Results_wtDisease_map,paste(Root_location,"/OutputFiles/workflow4_allCTD_new_stdlifts_wtDiseaseMap.txt",sep=''), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
