...
####             using the annotation information from both ToxCast and CTD datasets.
####
####Notes: This file is essentially a commented and cleaned up version of 'Workflow5_Merge_analysis_2_gprm.R'
####       
####Potential issues:
...
######ToxCast
TC_FIM_analysis_genes <- read.table(paste(Root_location,"/OutputFiles/workflow4_Toxcast_ALLchem_assoc_rules_wtDiseaseMap.txt", sep=''), header=TRUE, sep='\t', comment.char='', quote='', na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
...
###Reading in the rules from the FIM analysis
######CTD (TC GENES)
CTDTC_FIM_analysis_genes <- read.table(paste(Root_location,"/OutputFiles/workflow4_TCgenes_in_CTD_ALLchem_assoc_rules_wtDiseaseMap.txt", sep=''), header=TRUE, sep='\t', comment.char='', quote='', na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
##merging the gene annotation data to the rules 
...
###Reading in the rules from the FIM analysis
######CTD ALL
CTDALL_FIM_analysis_genes <- read.table(paste(Root_location,"/OutputFiles/workflow4_allCTD_ALLchem_assoc_rules_wtDiseaseMap.txt", sep=''), header=TRUE, sep='\t', comment.char='', quote='', na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
##merging the gene annotation data to the rules 
...
Uniprot_gene_map_data <- read.table(paste(Root_location,"/InputFiles/uniprot_gene_protein_map_CTD_chem_gene_ixns_Jan182018.tab", sep=''), header=TRUE, sep='\t', comment.char='', quote='', na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
Reactome_mapping_data <- read.table(paste(Root_location,"/InputFiles/Reactome_uniprot_protein_map_CTD_chem_gene_ixns_Jan182018_download.csv", sep=''), header=TRUE, sep=',', comment.char='', quote='', na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
...
merged_gene_protein_human<- unique(merge(Uniprot_gene_map_data, Reactome_mapping_data, by.x="Entry", by.y="Submitted.identifier"))
##merging the uniprot and reactome IDs to the FIM rules and annotation data
###Toxcast
TC_merged_with_GeneID <- merge(TC_FIM_genes_for_uniprot_mapping,merged_gene_protein_human, by.x= "intended_target_entrez_gene_id", by.y="yourlist.M20180417A7434721E10EE6586998A056CCD0537E5F78200") 
...
###CTD (TC GENES)
CTDTC_merged_with_GeneID <- merge(CTDTC_FIM_genes_for_uniprot_mapping,merged_gene_protein_human, by.x= "GeneID", by.y="yourlist.M20180417A7434721E10EE6586998A056CCD0537E5F78200") 
...
###CTD ALL
CTDALL_merged_with_GeneID <- merge(CTDALL_FIM_genes_for_uniprot_mapping,merged_gene_protein_human, by.x= "GeneID", by.y="yourlist.M20180417A7434721E10EE6586998A056CCD0537E5F78200") 
...
##TOXCAST
TC_Gene_protein <- TC_merged_with_GeneID #read.table(paste(Root_location,"/IntermediateData/Gene_protein_maps_TC_FIMresults.csv", sep=''),header=TRUE, sep=',', comment.char='', quote="", na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
...
write.table(TC_merged_gene_protein_reactome,file=file.path(outDir, "TC_ALLchem_Gene_protein_pathway_maps.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
##CTD (TC GENES)
CTDTC_Gene_protein <- CTDTC_merged_with_GeneID #read.table(paste(Root_location,"/IntermediateData/Gene_protein_maps_CTDTC_FIMresults.csv", sep=''),header=TRUE, sep=',', comment.char='', quote="", na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
##merging this with the merged FIM annonation and protein file from above
...
write.table(CTDTC_merged_gene_protein_reactome,file=file.path(outDir, "CTDTC_ALLchem_Gene_protein_pathway_maps.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
##CTD ALL
CTDALL_Gene_protein <- CTDALL_merged_with_GeneID #read.table(paste(Root_location,"/IntermediateData/Gene_protein_maps_CTDALL_FIMresults.csv", sep=''),header=TRUE, sep=',', comment.char='', quote="", na.strings = "",fill=TRUE, strip.white=TRUE, colClasses="character")
##merging this with the merged FIM annonation and protein file from above
...
write.table(CTDALL_merged_gene_protein_reactome,file=file.path(outDir, "CTDALL_ALLchem_Gene_protein_pathway_maps.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
...
write.table(TC_gene_protein_pathway_disease, file=file.path(outDir2, "TC_ALLChem_Gene_protein_pathway_disease_associations.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)

#CTDTC_merged_gene_protein_reactome2 <- unique(CTDTC_merged_gene_protein_reactome[,c(-1,-4)])
...
write.table(CTDTC_gene_protein_pathway_disease, file=file.path(outDir2, "CTDTC_ALLChem_Gene_protein_pathway_disease_associations.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)

CTDALL_merged_gene_protein_reactome2 <- unique(CTDALL_merged_gene_protein_reactome[,c(-1,-4)])
CTDALL_gene_protein_pathway_disease <- unique(merge(CTDALL_FIM_analysis_genes, CTDALL_merged_gene_protein_reactome2, by.x="lhs", by.y="intended_target_gene_symbol"))
write.table(CTDALL_gene_protein_pathway_disease, file=file.path(outDir2, "CTDALL_ALLChem_Gene_protein_pathway_disease_associations.txt"), sep='\t', col.names=TRUE, row.names=FALSE,quote=FALSE)
